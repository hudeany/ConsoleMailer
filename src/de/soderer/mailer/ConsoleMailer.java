 ...
 */
public class ConsoleMailer extends BasicUpdateableConsoleApplication {
				...
				for (int i = 0; i < arguments.size(); i++) {
					if ("help".equalsIgnoreCase(arguments.get(i)) || "-help".equalsIgnoreCase(arguments.get(i)) || "--help".equalsIgnoreCase(arguments.get(i)) || "-h".equalsIgnoreCase(arguments.get(i)) || "--h".equalsIgnoreCase(arguments.get(i))
						...
						if (arguments.size() > i + 2) {
							new ConsoleMailer().updateApplication(arguments.get(i + 1), arguments.get(i + 2).toCharArray());
						} else if (arguments.size() > i + 1) {
							new ConsoleMailer().updateApplication(arguments.get(i + 1), null);
						} else {
							new ConsoleMailer().updateApplication(null, null);
						}
						return 1;
				...
				if (Utilities.isBlank(arguments.get(i))) {
					...
					if (dkimPrivateKey != null) {
						...
						if (i >= arguments.size()) {
							...
							try {
								dkimPrivateKey = CryptographicUtilities.getRsaPrivateKeyFromString(FileUtilities.readFileToString(dkimPrivateKeyFile, charset == null ? StandardCharsets.UTF_8 : charset));
							} catch (@SuppressWarnings("unused") final Exception e) {
